...
#define read_uint32_bits(start,size) \
    ...
    void InitItemWeaponTypesData(const ITxtReader& txtReader)
    {
        ...
        for (size_t i = 0; i < numRows; ++i)
        {
            ...
            if (!strValue.empty() && (strValue != "0"))
            {
                if (code != "leg") // Writ's leg is not marked as a "Quest" item
                {
    ...
    void ProcessMagicalProperites(const std::vector<ItemAffixModType>& modTypes, std::vector<MagicalAttribute>& magicalAttributes, ItemRandStruct& rnd, d2ce::EnumItemVersion version, std::uint16_t gameVersion)
    {
        ...
        if (bHasChained)
        {
            ...
            magicalAttributes.erase(std::remove_if(magicalAttributes.begin(), magicalAttributes.end(),
                [](const MagicalAttribute& x) {return x.Name.empty(); }), magicalAttributes.end());
        }
